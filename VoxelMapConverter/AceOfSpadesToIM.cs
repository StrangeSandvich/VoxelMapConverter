...
namespace VoxelMapConverter
{
    class AceOfSpadesToIM
    {
        ...
        private const int aosSizeZ = 64;
        public static IntermediateMap ToIntermediateMap(string aosFilename)
        {
            IntermediateMap mapResult = new IntermediateMap(aosSizeX, aosSizeY, aosSizeZ, Block.AOSSOLID);
            
            byte[] AoSMapData = File.ReadAllBytes(aosFilename);
            int columnStart = 0;
            for (int x = 0; x < aosSizeX; x++)
            {
                for (int y = 0; y < aosSizeY; y++)
                {
                    ...
                    while (true)
                    {
                        ...
                        //TODO some stupid issue with top_color_start and end values being above 200
                        if(top_color_start > 64)
                        {
                            top_color_switched = 0;
                        }
                        else
                        {
                        ...
                        //Air run. Start at the latest z and got down to top_color_start
                        ...
                        spanStart = spanStart + 4 * number_4_byte_chunks;
                    }
                }
            }
